...
export default function Search({ q }) {
    ...
    useEffect(() => {
        q ? fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${q}&key=${api[3]}&maxResults=25`)
    ...
    return (
        <div className='search'>
            ...
            {data ?
                ...
                <a key={i} target='_blank' rel="noopener noreferrer" href={'https://www.youtube.com/watch?v='+el.id.videoId}><VideoCard kind={el.id.kind} thumbnail={el.snippet.thumbnails.medium.url} title={el.snippet.title} channelTitle={el.snippet.channelTitle} description={el.snippet.description} /></a>) 
                    : null}
